...
const KidView: React.FC<KidViewProps> = ({ lesson, onCompleteActivity, activityScores }) => {
  ...
  if (showLessonSummary) {
    return (
      <div className="bg-white rounded-3xl p-8 text-center shadow-xl border-4 border-indigo-100 animate-bounce-in">
        <div className="w-24 h-24 bg-amber-100 text-amber-500 rounded-full flex items-center justify-center mx-auto mb-6">
          <Trophy size={48} />
        </div>
        <h2 className="text-3xl font-bold text-indigo-700 mb-2">Tuyệt vời ông mặt trời!</h2>
        <p className="text-slate-600 mb-8">Con đã hoàn thành xuất sắc bài: <br/> <span className="font-bold text-slate-800">{lesson.title}</span></p>
  ...
  return (
    <div className="space-y-6">
      {/* Intro Section */}
      <div className="bg-white p-6 rounded-3xl shadow-sm border-b-4 border-indigo-200">
        <div className="flex flex-col md:flex-row gap-6 items-center">
          <Mascot 
            state={feedback.type === 'success' ? 'happy' : feedback.type === 'hint' ? 'thinking' : 'idle'} 
            dialogue={feedback.text || `Chào con! Hôm nay mình học ${lesson.title} nhé!`} 
          />
          <div className="flex-1">
            <h2 className="text-2xl font-bold text-indigo-700 mb-2 flex items-center gap-2">
              {lesson.icon} {lesson.title}
            </h2>
            <div className="prose prose-indigo" dangerouslySetInnerHTML={{ __html: lesson.raw_html }} />
          </div>
        </div>
      </div>
        ...
        {lesson.activities.map((a, idx) => (
          <div 
            ...
            className={`h-2.5 rounded-full transition-all duration-300 ${
              idx === currentIdx ? 'w-8 bg-indigo-600' : 
      ...
      {/* Activity Content */}
      <div className="bg-white rounded-3xl shadow-lg p-6 md:p-10 border-t-4 border-indigo-500 min-h-[400px] flex flex-col">
        <div className="flex items-center gap-2 mb-6">
          <div className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs font-bold uppercase tracking-wider">
            Hoạt động {currentIdx + 1}
          </div>
        ...
        <div className="mt-8 flex justify-end">
          {isCurrentDone && !isLastActivity && (
            <button 
              ...
              className="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg hover:shadow-indigo-200 transition-all transform hover:-translate-y-1"
            >
